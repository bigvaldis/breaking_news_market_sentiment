...
export default function Sp500Chart({ data = [] }) {
  ...
  useEffect(() => {
    ...
    if (hasOHLC) {
      ...
      chart.addLineSeries({ color: '#10b981', lineWidth: 2 }).setData(seriesData.map((d) => ({ time: d.time, value: d.close })))
    }
  ...
  return (
    <div className="chart-card candle-chart-card" data-asset="sp500">
      <h3>S&P 500 (^GSPC)</h3>
